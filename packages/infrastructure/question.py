...
class InMemoryQuestionRepository:
    ...
    async def add_bank(self, bank: QuestionBank) -> None: self.banks[bank.id] = bank
    async def get_bank(self, ident: UUID) -> QuestionBank | None: return self.banks.get(ident)
    async def banks_for_project(self, project: UUID) -> list[QuestionBank]: return [b for b in self.banks.values() if b.project_id == project]
    async def add_version(self, version: QuestionBankVersion) -> None: self.versions[version.id] = version
    async def get_version(self, ident: UUID) -> QuestionBankVersion | None: return self.versions.get(ident)
    async def versions_for_bank(self, bank: UUID) -> list[QuestionBankVersion]: return sorted((v for v in self.versions.values() if v.question_bank_id == bank), key=lambda v: v.version_number)
    async def add_blueprint(self, blueprint: QuestionBlueprint) -> None:
        if not any(x.fingerprint == blueprint.fingerprint for x in self.blueprints[blueprint.generation_job_id]): self.blueprints[blueprint.generation_job_id].append(blueprint)
    async def add_question(self, question: Question) -> None: self.questions[question.id] = question
    async def get_question(self, ident: UUID) -> Question | None: return self.questions.get(ident)
    async def questions_for_version(self, version: UUID) -> list[Question]: return [q for q in self.questions.values() if q.question_bank_version_id == version]
    ...
    async def add_decision(self, decision: QuestionReviewDecision) -> None: self.decisions[decision.question_id].append(decision)


class DisabledCodeExecutionSandbox:
    async def execute(self, *_: Any, **__: Any) -> None: raise QuestionError("CODE_SANDBOX_UNAVAILABLE", "Code execution sandbox is disabled")
class DisabledSQLExecutionSandbox:
    async def execute(self, *_: Any, **__: Any) -> None: raise QuestionError("SQL_SANDBOX_UNAVAILABLE", "SQL execution sandbox is disabled")
